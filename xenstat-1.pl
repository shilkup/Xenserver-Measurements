...
# perl xenstat.pl d
#  
#  Disk I/O (R+W reqs)  52_pyth 54_garu 59_gyrf   Dom-0
...
my @domuptime =();
my $dat = `$XL list`
      or die "no output from $XL list ?? maybe it isn't in your path?";
  ...
  foreach my $line (@result)
  {
   ...
   if ($xenInfo[0] eq 'Domain-0') {$xenInfo[0]='Dom-0';}
    if ($mode eq 'c') {
      ...
      push(@cpa, $xenInfo[$zcpu]);      #cpu
      push(@cpa, $xenInfo[$zmemu]);      #mem
      push(@cpa, $xenInfo[$zrd+$o]);    #io rd
      push(@cpa, $xenInfo[$zwr+$o]);    #io wr
      push(@cpa, $xenInfo[$znrd+$o]);   #network tx
      push(@cpa, $xenInfo[$znwr+$o]);   #network rx
      $nCols = 6;       #number of colums
    }
     ...
     if (!$printhdr) {
        ...
        if ($mode eq 'x') {
            for(my $j=0; $j<$count; $j++){
              $buf .= sprintf " %8.1f", $cpa[$j*$nCols+0];
              $buf .= sprintf " %8.1f", $cpa[$j*$nCols+1];
              $buf .= sprintf " %8.1f", ($cpa[$j*$nCols+2] - $prevcpa[$j*$nCols+2]);
              $buf .= sprintf " %8.1f", ($cpa[$j*$nCols+3] - $prevcpa[$j*$nCols+3]);
              $buf .= sprintf " %8.1f", ($cpa[$j*$nCols+4] - $prevcpa[$j*$nCols+4]);
              $buf .= sprintf " %8.1f", ($cpa[$j*$nCols+5] - $prevcpa[$j*$nCols+5]);
              $buf .= sprintf " %8s", "|";
            }
   ...
   print "$buf\n";
    ...
    if ($printhdr) {
     if ($idling) {
        ...
        elsif ($mode eq 'd') {printf "%20s",'Disk I/O (R+W reqs)  ';}
        elsif ($mode eq 'x') {printf "%10s",'All metrics  ';}
     }
